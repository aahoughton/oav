...
| `custom-keywords.ts`      | Register a schema keyword (`activeTenant`) via the `keywords` opt |
...
- Each example imports from `packages/*/src` directly, so you don't need
  to `pnpm build` first.
- Inline OpenAPI documents keep everything in one file — swap the inline
  doc for `resolveSpec(...)` to read from disk.
