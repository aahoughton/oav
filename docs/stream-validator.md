...
the OAV monorepo that reuses OAV's error model and delegates ambiguous
subtrees to OAV's in-memory compiler (which is where format assertion and
the built-in formats live).
...
- **Package:** `@oav/stream-validator` (published `@aahoughton/oav-stream-validator`),
  ...
  depending on `core` + `schema` (no cycle). `formats` reaches it only
  through the `schema` delegate; `spec` (`resolveSpec()`) is a caller-side
  touch point, not a package dependency.
...
| `format`                                             | STREAM / BUFFER          | scalar end                  | token-close                  | annotation-only (STREAM) unless the dialect asserts `format`, then the scalar is buffered and validated by the in-memory delegate (built-in formats from `@oav/formats` live there) |
...
| ---------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `@oav/formats`   | Indirect | Not a direct dependency. Built-in formats reach the validator through the `@oav/schema` BUFFER delegate; user `formats` are threaded into that compile. The forward STREAM path does not assert `format`.    |
